...
object Huffman {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def aux(ct: CodeTree, charsLeft: List[Char], acc: List[Bit]): List[Bit] = {
      charsLeft match {
        case c :: cs => 
          ct match {
            case Fork(left, right, _, _) => 
              if (chars(left).contains(c)) aux(left, charsLeft, 0 :: acc)
              else if (chars(right).contains(c)) aux(right, charsLeft, 1 :: acc)
              else aux(tree, cs, acc)
            case Leaf(char, _) => aux(tree, cs, acc)
          }
        case List() => acc
      }
    }
    
    aux(tree, text, List()).reverse
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def aux(ct: CodeTree, bits: List[Bit], acc: CodeTable): CodeTable = {
      ct match {
	    case Fork(left, right, chars, _) => aux(left, 0 :: bits, aux(right, 1 :: bits, acc))
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def aux(textLeft: List[Char]): List[Bit] = {
      textLeft match {
        case c :: cs => codeBits(codeTable)(c) ++ aux(cs)
        case List() => List()
      }
    }
